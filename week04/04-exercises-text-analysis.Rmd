...
raw <- read_file(my.file, locale = locale(encoding="Shift_JIS"))
print(raw) # this is Japanese [language]
my.utf.file <- read_file(my.file, locale = locale(encoding="Shift_JIS"))
write_file(my.utf.file, paste0(ddir, "news-article-1-UTF-8.txt"))
```

```{r}
read_file(paste0(ddir, "news-article-1-UTF-8.txt"))
```
...
str_view(text, "[A-Z]+[0-9]+[A-Z]? *[0-9]?[A-Z]+") 
```
