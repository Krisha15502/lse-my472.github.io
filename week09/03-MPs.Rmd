...
root <- "~/Library/CloudStorage/OneDrive-LondonSchoolofEconomics/MY472/Website/week09/seminar9" 
ddir <- "~/Library/CloudStorage/OneDrive-LondonSchoolofEconomics/MY472/Website/week09/seminar9/data"
odir <- "~/Library/CloudStorage/OneDrive-LondonSchoolofEconomics/MY472/Website/week09/seminar9/outputs"
...
results.file <- str_extract(results.url, "/([^/]+)$") # get the file name from the link (see above)
...
if(!file.exists(results.file)) { download.file(results.url, results.file) }
```

Now, load the data as a tibble called `rf` and print the `head()` to see how it looks.

```{r}
rf <- read_csv(results.file)
head(rf)
```
...
if(!file.exists(borders.file)) { download.file(borders.data, borders.file) }

## Load the data and check it's right
gf <- st_read(borders.file)
print(gf)
plot(gf$geometry) # Hint: only want to plot the vector map data
```
...
scotland <- st_crop(gf, ymin = bounds$ymin, ymax = bounds$ymax, xmin = bounds$xmin, xmax = bounds$xmax) # hint: Google the function for cropping a map in sf
...
head(gf)
raw.html <- read_html("https://en.wikipedia.org/wiki/Parliamentary_constituencies_in_London") 
```
...
london.consts <- raw.html %>% 
  html_table() %>%
  .[[3]] ## Which table is the one on the page with the constituency names?

## extract names
london.consts <- london.consts$Constituency
length(london.consts) # check there are 75, which there should be
```
...
gf <- gf %>%
  mutate(london = ifelse(str_to_lower(gf$`PCON24NM`) %in% str_to_lower(london.consts), 1, 0)) 
...
gf$london[str_detect(gf$PCON24NM, "Edmonton and Winchmore Hill")] <- 1 # recode it

sum(gf$london) # now there are 75

## Plot only London constituencies
plot(gf$geometry[gf$london==1])
```
...
```{r}
df <- full_join(gf, rf, by = c("PCON24CD" = "ONS ID")) # Hint: use a "full join" from dplyr here
head(df)
```
...
wf <- wf %>% 
  group_by(PCON24CD) %>% 
  mutate(win_margin = round(percent - lead(percent), 4)) %>% # Hint: how do you subtract the value in the next row from the value in current row in dplyr? Google it!
  ungroup() %>%
  select(PCON24CD,PCON24NM,london,percent,win_margin,`Party abbreviation`,geometry) # Hint: only keep columns we need
