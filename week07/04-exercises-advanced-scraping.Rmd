...
src <- driver$getPageSource() # Hint: look up how to get the page source in the help documents for RSelenium: https://cran.r-project.org/web/packages/RSelenium/index.html
...
accept_button <- driver$findElement(using = "xpath", value = "/html/body/div/div[2]/div[3]/div/div/button[1]")
...
search_field_xpath <- "/html/body/main/article/div/div/div/article/div/div/div/figure/figure/div/div/section[3]/div[2]/div/div/div/input" 
...
```{r}
constituency_xpath <- ________
constituency <- driver$________(using="xpath", value=constituency_xpath)
...
extract_results <- function(){
  results <- driver$getPageSource()[[1]] %>%
    read_html() %>% 
    ________() # <- extract the tables

  results <- results[____][[1]] # Which of the tables extracted is the correct one?
...
close_results <- ________

# 2. Then find and click it
driver$findElement(using="xpath", value=close_results)$________()
Sys.sleep(0.5)
...
get_data <- function(search_term, wait_secs = 5){
  ...
  for(i in sugg_list){
    ...
    constituency <- driver$________(using="xpath", value=constituency_xpath)
    constituency$________()
    Sys.sleep(wait_secs)
    all_results <- bind_rows(________, extract_results())
  }
  return(all_results)
}
```
...
const <- read_html(url_const) %>%
  ________(fill = TRUE) 
const <- const[[____]] ## Extract the list of English constituencies; we could also get Scottish, Welsh, and NI constituencies by finding their tables on the same page -- you can do this as a challenge if you want!
